...
# --- JSON variants for agent mode (no YAML round-trip) ---


async def stream_diagnostics_json(
    connection,
) -> AsyncIterator[list[DiagnosticItem]]:
    """Stream diagnostics via agent JSON subscription (no YAML parsing).
    ...
    if not isinstance(connection, AgentConnection):
        raise TypeError(f"Expected AgentConnection, got {type(connection).__name__}")
