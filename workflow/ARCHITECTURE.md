...
- `AgentConnection` — WebSocket клиент к monitoring_agent (JSON-RPC 2.0)
  - `subscribe_json(channel, params)` — прямой JSON streaming (logs, diagnostics, echo)
  - `exec_stream(cmd)` — только для topic.hz (backward compat)
  - `_call(method, params)` — JSON-RPC request/response
  - Channel-dependent queue sizes: logs=2000, diagnostics=1000, echo=200, hz=100
...
- `LogCollector` — стриминг логов из `/rosout`
  - Agent mode: `subscribe_json('logs')` → `_parse_json_log(dict)` → LogMessage напрямую
  - Docker mode: `exec_stream` → YAML → regex → LogMessage
- `DiagnosticsCollector` — стриминг `/diagnostics` топика
  - Agent mode: `stream_diagnostics_json()` → subscribe_json → DiagnosticItem напрямую
  - Docker mode: `stream_diagnostics()` → exec_stream → YAML → yaml.safe_load → DiagnosticItem
  - Также: `stream_mrm_status_json`, `stream_mrm_state_json`, `stream_bool_topic_json`
- `TopicHzMonitor` — on-demand мониторинг Hz по группам топиков (shared cache между клиентами)
- `SharedEchoMonitor` — shared echo с fan-out (одна подписка на топик, broadcast всем клиентам)
  - Agent mode: `subscribe_json('topic.echo')` → JSON dict
  - Docker mode: `exec_stream` → YAML text
  - Ref-counting подписчиков: задача создаётся при первом клиенте, отменяется при уходе последнего
  - Truncation: сообщения >10KB обрезаются
- `DroppableQueue` — обёртка asyncio.Queue, считающая дропнутые сообщения при QueueFull
...
- `websocket.py` — WebSocket эндпоинты:
  ...
  - `/ws/logs/{node}`, `/ws/logs/all` — логи (per-node и unified), DroppableQueue
  - `/ws/diagnostics` — диагностики (auto-detect agent mode для JSON path)
  - `/ws/topics/hz`, `/ws/topics/hz-single/{topic}` — мониторинг Hz
  - `/ws/topics/echo/{group_id}`, `/ws/topics/echo-single/{topic}` — echo через SharedEchoMonitor
  - `/ws/alerts` — алерты
...
- `JsonView` — structured JSON view для echo сообщений (ключи синие, числа зелёные, строки оранжевые)
...
├── ws_server.py            # WebSocket сервер, JSON-RPC dispatch, _ClientWriter с PriorityQueue
...
- Приоритизация: logs/diagnostics (priority=0) > hz (1) > echo (2)

---

## Конфигурация

- `config/servers.yaml` — серверы (type: local/ssh/agent, agent_url)
